...
const {form, loading} = useTypedSelector((state) => state.reducers.ProductTypes);
    ...
    <Formik
            ...
            onSubmit={async values => {
              const navi = '/product/product-type'
                if(params.id !== undefined){
                  dispatch(
                    updateProductType(values, routeLink.Product.ProductType)
                  )
                  navigate(routeLink.Product.ProductType)
                }else{
                  dispatch(
                    postProductType(values, routeLink.Product.ProductType)
                  )
        ...
        >
            ...
            {props => {
                ...
                if (reload === false && values.id === 0 && params.id !== undefined && Object.keys(form).length > 0) {
                    setValues({
                        ...values,
                        ...form
                    });
                    setReload(true)
                    console.log('ProductType: ', form)
